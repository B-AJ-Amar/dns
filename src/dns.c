...
__uint8_t get_flag_qr(__uint16_t flags) { return (flags & DNS_QR_MASK) >> DNS_QR_SHIFT; }
__uint8_t get_flag_opcode(__uint16_t flags) {
    return (flags & DNS_OPCODE_MASK) >> DNS_OPCODE_SHIFT;
}
__uint8_t get_flag_aa(__uint16_t flags) { return (flags & DNS_AA_MASK) >> DNS_AA_SHIFT; }
__uint8_t get_flag_tc(__uint16_t flags) { return (flags & DNS_TC_MASK) >> DNS_TC_SHIFT; }
__uint8_t get_flag_rd(__uint16_t flags) { return (flags & DNS_RD_MASK) >> DNS_RD_SHIFT; }
__uint8_t get_flag_ra(__uint16_t flags) { return (flags & DNS_RA_MASK) >> DNS_RA_SHIFT; }
__uint8_t get_flag_z(__uint16_t flags) { return (flags & DNS_Z_MASK) >> DNS_Z_SHIFT; }
__uint8_t get_flag_rcode(__uint16_t flags) { return (flags & DNS_RCODE_MASK) >> DNS_RCODE_SHIFT; }
...
static dns_rr **parse_rr_section(char *buffer, size_t len, unsigned long *cursor,
                                 __uint16_t count) {
    if (count == 0)
        return NULL;
...
static dns_question **parse_question_section(char *buffer, size_t len, unsigned long *cursor,
                                             __uint16_t count) {
    if (count == 0)
        return NULL;
